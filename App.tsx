...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const checkShared = async () => {
            ...
            if (sharedId) {
                try {
                    const docSnap = await getDoc(doc(db, "slideshows", sharedId));
                    if (docSnap.exists()) {
                        const data = { id: docSnap.id, ...docSnap.data() } as SavedSlideshow;
                        loadProject(data);
                    }
                } catch (e) {
                    setError("Could not load shared project.");
                }
            }
        };
        checkShared();
                                        ...
                                        {mediaWithTimestamps.map((m, i) => (
                                            <div key={m.id} className="h-10 bg-gray-800 rounded-md overflow-hidden border border-gray-700 relative group flex-shrink-0" style={{ width: `${(m.timelineEnd! - m.timelineStart!) * 20}px` }}>
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 transition-opacity">
                                                    <span className="text-[8px] font-bold text-white">{settings.interval}s</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <section className="bg-gray-800/30 p-8 rounded-[3.5rem] border border-gray-700/50 relative overflow-hidden">
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 relative z-10">
                                        ...
                                        <div className="flex gap-2 shrink-0">
                                            ...
                                            <button onClick={() => { setElapsedTime(0); setIsPlaying(true); }} className="bg-brand-purple text-white px-10 py-3 rounded-full text-xs font-bold shadow-2xl shadow-brand-purple/30 hover:scale-105 active:scale-95 transition-all">Preview Fullshow</button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="aspect-video bg-gray-950 rounded-[3rem] relative overflow-hidden border border-gray-800 shadow-2xl flex items-center justify-center group">
                                        {mediaFiles.length > 0 ? (
                                            ...
                                            <div className="text-center p-12">
                                                ...
                                                <p className="text-xs text-gray-600 uppercase font-black tracking-widest">Workspace Empty</p>
                                            </div>
                                        )}
                                    </div>
                                </section>
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
                            {!appleMusicAuthorized && !isSimulationMode ? (
                                <div className="text-center py-20 bg-gray-900/50 rounded-[3rem] border border-gray-800">
                                    ...
                                    <h3 className="text-xl font-bold mb-4">Apple Music Library</h3>
                                    <p className="text-xs text-gray-500 mb-10 max-w-xs mx-auto">Authorize to browse your personal playlists and library directly in the editor.</p>
                                    <button onClick={handleAuthorizeApple} className="bg-apple-red text-white px-12 py-4 rounded-full font-bold shadow-2xl shadow-apple-red/30 hover:scale-105 transition-all">Authorize</button>
                                ...
                                <div className="grid gap-4">
                                    ...
                                    {MOCK_TRACKS.map(track => (
                                        <div 
                                            key={track.id} 
                                            className="bg-gray-900/60 p-5 rounded-[2rem] border border-gray-800 flex justify-between items-center hover:border-brand-purple group cursor-pointer transition-all"
                                            onClick={() => {
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-gray-800 rounded-2xl flex items-center justify-center text-xl group-hover:scale-110 transition-transform">🎵</div>
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-gray-200">{track.name}</span>
